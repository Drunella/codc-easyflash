...
def apply_patch_hex(data): 
    ...
    # address = [the local address in the file] ; load address is automatically applied
    # original = [the hex data of the old value]
    # new = [the hex data of te new value]
    global binary_file
    offset = int(data["offset"], 0)
...
def main(argv):
    ...
    p.add_argument("-q", dest="dryrun", action="store_true", required=False, help="don't patch, just dryrun.", default=False)
    ...
    if (args.verbose):
        if dryrun is False:
            ...
            print("patching binary files (dry run)")
